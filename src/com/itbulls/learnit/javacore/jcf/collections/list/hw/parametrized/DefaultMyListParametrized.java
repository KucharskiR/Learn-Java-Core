...
public class DefaultMyListParametrized<T> implements MyListParametrized<T>, ListIterableParametrized<T> {
	 ...
	 */
	@Override
	public boolean containsAll(MyListParametrized<?> c) {
	...
	@Override
	public ListIteratorParametrized<T> listIterator() {
	 ...
	 * 
	 */
	private class ListIteratorImpl<T> extends IteratorImpl<T> 
									implements ListIteratorParametrized<T> {
	...
	private class IteratorImpl<T> implements Iterator<T> {
		...
		public void remove() {
			...
			DefaultMyListParametrized.this.removeNodeByIndex(lastRet);
			if (lastRet < cursor) {
				cursor--;
			}
            lastRet = -1;
